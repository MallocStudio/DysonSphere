...
public class physButton : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        if(transform.eulerAngles != Vector3.zero)
        {
            ...
            transform.eulerAngles = Vector3.zero; //change base of button to be zero
            upperLowerDiff = buttonUpperLimit.position.y - buttonLowerLimit.position.y;
            transform.eulerAngles = savedAngle; // reset the button at the start
        }
        upperLowerDiff = buttonUpperLimit.position.y - buttonLowerLimit.position.y;
    }
